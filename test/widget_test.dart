...
import 'package:mahjong_mate/shared/auth/auth_user_provider.dart';

void main() {
  testWidgets('Rule set list shows empty state', (WidgetTester tester) async {
    const ownerUid = 'test-owner-uid';
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          currentUserUidProvider.overrideWith((ref) => ownerUid),
          followedRuleSetsByOwnerProvider(
            ownerUid,
          ).overrideWith((ref) => Stream<List<RuleSet>>.value(const [])),
        ],
    ...
    expect(find.text('フォロー中のルールセットがありません。'), findsOneWidget);
  });
}
